...
} from '@/hooks/useContactColumnPrefs';

export function ContactColumnToggle() {
  const { visible, toggle, reset } = useContactColumnPrefs();
    ...
    <div ref={wrapRef} className="relative">
      ...
      >
        ...
        <span className="text-caption text-neutral font-mono font-tabular">
          {visible.size}/{ALL_CONTACT_COLUMNS.length}
        </span>
      </button>
      {open && (
        <div className="absolute right-0 mt-1 z-30 bg-surface border border-border rounded-md shadow-e2 min-w-[200px] overflow-hidden">
          ...
          {ALL_CONTACT_COLUMNS.map((c) => (
            ...
            >
              <input
                type="checkbox"
                checked={visible.has(c.key as ContactColumnKey)}
                onChange={() => toggle(c.key as ContactColumnKey)}
